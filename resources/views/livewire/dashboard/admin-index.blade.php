        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="bg-gradient-to-tr from-emerald-50 to-emerald-500 ring-1 ring-emerald-500 shadow-md rounded-md p-4">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 place-items-center">
                    <div class=" bg-gradient-to-b from-slate-100/70 to-zinc-100 rounded-lg p-2 w-full text-center h-24 border border-black shadow-md">
                        <span class="block text-2xl lg:text-4xl mb-2"> {{ $inventories->count() }}</span>
                        <p>Total Inventories</p>
                    </div>
                    <div class=" bg-gradient-to-b from-slate-100/70 to-zinc-100 rounded-lg p-2 w-full text-center h-24 border border-black shadow-md">
                        <span class="block text-2xl lg:text-4xl mb-2">{{ count($inventories->where('inventoryable_type', '=', 'App\Models\CraneInventory')) }}</span>
                        <p>Total Cranes</p>
                    </div>
                    <div class=" bg-gradient-to-b from-slate-100/70 to-zinc-100 rounded-lg p-2 w-full text-center h-24 border border-black shadow-md">
                        <span class="block text-2xl lg:text-4xl mb-2">{{ count($inventories->where('inventoryable_type', '=', 'App\Models\EquipmentInventory')) }}</span>
                        <p>Total Equipment</p>
                    </div>
                    <div class=" bg-gradient-to-b from-slate-100/70 to-zinc-100 rounded-lg p-2 w-full text-center h-24 border border-black shadow-md">
                        <span class="block text-2xl lg:text-4xl mb-2">{{ count($inventories->where('inventoryable_type', '=', 'App\Models\PartInventory')) }}</span>
                        <p>Total Parts</p>
                    </div>
                    <div class="bg-gradient-to-b from-slate-100/70 to-zinc-100 rounded-lg p-2 w-full text-center h-24 border border-black shadow-md col-span-4">
                        <span class="block text-2xl lg:text-4xl mb-2">{{ucwords(Str::replace("_", " ", $mostCommonType))}}</span>
            ...
            <div class="bg-gradient-to-tr from-sky-50 to-cyan-500 shadow-md rounded-md p-4 ring-1 ring-cyan-600">
                ...
                <div class="bg-white p-2 rounded-lg border-b flex justify-center items-center mb-2">
                    <div class="w-1/3 text-start">Temporary Upload Size</div>
                ...
                <div class="bg-white p-2 rounded-lg border-b flex justify-between items-center">
                    <span class='w-2/3'>Total Images on Server:</span>
