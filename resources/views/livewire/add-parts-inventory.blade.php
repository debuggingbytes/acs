<div class="rounded-md px-2 py-4">
  ...
  <form class="block w-100" wire:submit='save'>
    <h3 class="uppercase block text-center rounded-lg py-2 bg-slate-200 shadow-md mb-5">Parts Information</h3>
    ...
    <div class="flex flex-col md:flex-row items-center py-2 relative">
        <label for="type" class="text-white font-bold text-lg uppercase bg-cyan-600 px-3 rounded-l-lg h-12 flex items-center justify-center w-full md:w-1/5">Parts Type</label>
        <select id="type" name="type" wire:model.live='type' class="rounded-r-lg border-gray-300 text-black shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block w-full p-3 bg-white h-12 appearance-none">
            <option selected disabled value="null">Select Parts Type</option>
            @if(!empty($types))
                @foreach ($types as $parts)
                    <option value="{{$parts->type}}" class="relative">{{$parts->text}}</option>
                @endforeach
            @endif
        </select>
    ...
    {{--  --}}
    @if ($addNewPartsTypeShow)
    <div class="my-10 ring-2 ring-red-500 bg-white shadow-xl rounded-lg ">
        <div class="block">
            <x-forms.label-input name="newtype" label="Add Category" livewire="addNewPartsType" />
        </div>
        <button class="bg-cyan-600 text-white uppercase font-bold block w-full text-center py-2.5 rounded-full" wire:click.prevent='addPartsType'>Add Category</button>
    </div>
    @endif
    ...
    <div class="flex items-center justify-center w-full">
        <button class='rounded-xl px-4 py-2 bg-cyan-500 text-white uppercase text-lg' wire:click='descriptionLiveToggle'>Enable Live View</button>
    </div>
    ...
    <div class="block rounded-md mb-4 w-full bg-white">
        <div class="flex items-center justify-center">
          <label for="files" class="w-1/5  h-40 text-white font-bold bg-cyan-600 py-2 px-4 rounded-l-md cursor-pointer hover:bg-cyan-700 flex items-center justify-center">
            ...
            <input type="file" id="files" multiple wire:model="images" class="hidden">
          </label>
